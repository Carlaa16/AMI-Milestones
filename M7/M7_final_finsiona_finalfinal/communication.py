...
class COMMS:
    def __init__(self, Inputs):
        ...
        self.tx_power_dbw_up = float(Inputs["Potencia transmisor satélite (dBm):"])   # Potencia inicial del transmisor en dBW
        self.tx_power_dbw_down = float(Inputs["Potencia transmisor estación (dBm):"])  # Potencia inicial del transmisor en dBW
        ...
        # Validar y convertir los tamaños de los mensajes
        try:
            uplink_data_size_kb = float(Inputs["Tamaño mensaje satélite:"])  # Convertir a float
            if uplink_data_size_kb < 0:
                self.gui.show_popup('El tamaño del mensaje del satélite debe ser mayor o igual a 0.', 'Input error')
                return

            downlink_data_size_kb = float(Inputs["Tamaño mensaje estación:"])  # Convertir a float
            if downlink_data_size_kb < 0:
                self.gui.show_popup('El tamaño del mensaje de la estación debe ser mayor o igual a 0.', 'Input error')
                return
        except ValueError:
            self.gui.show_popup('El tamaño del mensaje debe ser un número válido.', 'Input error')
            return 

        workspace_path = Inputs["Workspace:"]        
        output_path = Inputs["Output:"]
        if not output_path :
            self.gui.show_popup(f'Especifique una dirección para el workspace', 'Input error')
            return 

        os.makedirs(output_path, exist_ok=True)
        
        uplink_data_size_bits = uplink_data_size_kb * 1024 * 8
        downlink_data_size_bits = downlink_data_size_kb * 1024 * 8

       # Directorios y archivos
        sat_files_dir = os.path.join(workspace_path, "CleanedReports")
...
# Verificar visibilidad entre satélites y dispositivos IoT
        ...
        self.plot_iot_fov(iot_df, beamwidth, background_image_path, output_path, satellite_positions_copia)
        self.plot_satellite_fov(satellite_positions_copia, fov_angle, background_image_path, output_path)     
...
####################################### Visualización de datos #####################################
    def plot_satellite_fov(self, satellite_positions_copia, fov_angle, background_image_path, output_path):
        ...
        plt.legend(["FOV Satélites"], loc='upper right')
        image_path = os.path.join(output_path, "FOV_Satélites.png")
        plt.savefig(image_path)
        self.gui.show_image_popup(image_path)

    
    def plot_iot_fov(self, iot_df, beamwidth, background_image, output_path, satellite_positions_copia):
        ...
        ax.imshow(background, extent=[-180, 180, -90, 90], aspect='auto')
        if satellite_positions_copia:
            self.plot_traces(ax, satellite_positions_copia, color="red")
        ...
        plt.legend(["FOV IoT"], loc='upper right')
        image_path = os.path.join(output_path, "FOV_IoT.png")
        plt.savefig(image_path)
        self.gui.show_image_popup(image_path)
