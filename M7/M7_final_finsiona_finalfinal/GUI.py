...
class GMATGUI(Widget.QMainWindow):
    def __init__(self):
        ...
        self.add_input_decimal("Tamaño mensaje satélite:",self.comms_sat_layout, initial_value="100")
        ...
        self.add_input_decimal("Tamaño mensaje estación:",self.comms_iot_layout, initial_value="10000000")
        ...
        self.add_input_list("Zona a estudiar:", \
                    ["AFRICA", "ANTARCTICA", "Arctic Ocean", "ASIA", "AUSTRALIA", \
                     "EUROPE", "INDIAN OCEAN", "Mediterranean Sea", "NORTH AMERICA", \
                     "North Atlantic Ocean", "North Pacific Ocean", "PENÍNSULA IBÉRICA", \
                     "SOUTH AMERICA", "South Atlantic Ocean", "South Pacific Ocean"], \
                    self.comms_zone_layout)
    ...
    def add_input_decimal(self, label_text, target_layout=None, initial_value="", min_value=-1e10, max_value=1e10, decimals=3):
        """
        Agrega un campo de entrada para números decimales con validación.
        
        :param label_text: Etiqueta descriptiva del campo.
        :param target_layout: Layout donde agregar el campo.
        :param initial_value: Valor inicial del campo (como string).
        ...
        input_field.setText(str(initial_value).replace(",", "."))  # Reemplaza comas por puntos en el valor inicial
        input_field.setPlaceholderText("Ingrese un número decimal")

        # Configurar validador
        validator = Gui.QDoubleValidator(min_value, max_value, decimals)
        validator.setNotation(Gui.QDoubleValidator.StandardNotation)
        input_field.setValidator(validator)

        # Convertir automáticamente comas en puntos mientras el usuario escribe
        input_field.textChanged.connect(lambda text: input_field.setText(text.replace(",", ".")))

        # Añadir al layout
        self.add_in_layout(target_layout, input_field, label)
        self.list_more_options(target_layout, input_field, label)

        # Guardar el campo en el diccionario de inputs
